...
public class CalendarPickerView extends ListView {
  ...
  List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
    ...
    while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
      ...
      //计算本周是第几周，内层循环结束后总是指向最后一个单元格
      if(startdata != null){
          startcal.setTime(startdata);

          int day = startcal.getFirstDayOfWeek();

          startcal.add(Calendar.DAY_OF_MONTH,-day + 1);

          int weekNum = 0;

          long deltasec = cal.getTime().getTime() - startcal.getTime().getTime();

          if(deltasec <= 0){
              weekNum = 0;
          }else{
              long days = deltasec/(1000*3600*24);
              weekNum = (int) (days/7) + 1;
          }

          weekCells.add(new MonthCellDescriptor(new Date(),false,false,false,false,true,weekNum, RangeState.NONE));
      }
